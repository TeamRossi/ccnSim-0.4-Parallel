...
void client_Window::initialize()
{
	...
	if (find(content_distribution::clients, content_distribution::clients + num_clients ,getNodeIndex()) != content_distribution::clients + num_clients)
	{
		 ...
		 * the zipf[0] vector to establish which content to take
		 */
...
void client_Window::request_file_window(unsigned long cNum)
{
	...
	for (int i=1; i<=currWinSize; i++)
	{
		...
		while(found)
		{
			//name = content_distribution::zipf[0].value(dblrand());
		...
		if (content_distribution::zipf[0].get_perc_aggr() < 1.0)
		{
			if (name == content_distribution::zipf[0].get_ID_aggr()+1) // In case of catalog aggregation, the vector
																	...
																	// the same ID.
			{
				...
				name = std::floor((rand() / ((double)RAND_MAX+1))*(content_distribution::zipf[0].get_catalog_card() - name + 1) + name);
			}
		}*/
...
{
	//name_t name = content_distribution::zipf[0].value(dblrand());
