...
bool ttl_cache::check_if_two_ttl()
{
	string decision_policy = getAncestorPar("DS");
...
double ttl_cache::get_avg_size()
{
	return avg_as_curr;
}

double ttl_cache::get_nameCache_avg_size()
{
	return twoTTLDecisor->get_avg_name_size();
}

double ttl_cache::get_max_size()
{
	return max_as;
}


double ttl_cache::get_tc_node()
{
	return tc_node;
}

double ttl_cache::get_tc_name_node()
{
	return twoTTLDecisor->get_tc_name();
}
...
void ttl_cache::extend_sim()
{
	if(avg_as_curr!=0)
	{
		if(change_tc)
		{
			double tc_new = tc_node + tc_node*(target_cache*(1./avg_as_curr) - 1);
			if(!std::isinf(tc_new))
				tc_node = tc_new;
		}
	}
